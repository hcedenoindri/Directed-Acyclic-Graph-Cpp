...
class graph {
  ...
  public:
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      while(!q.empty()) {
        ...
        u = q.front();
        next_dist = report[u].dist + 1;
        q.pop();

        // examine outgoing edges of u
        for(edge &e : vertices[u].outgoing) {
          v = e.vertex_id;
          if (report[v].state == DISCOVERED && report[v].dist == next_dist) {
            report[v].npaths += report[u].npaths;
          }
          else if(report[v].state == UNDISCOVERED) {
            report[v].dist = next_dist;
            report[v].pred = u;
            report[v].npaths = report[u].npaths;
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> &rpt, 
      ...
      backwards_path.push_back(dest);
      while (rpt[u].pred != u) {
        backwards_path.push_back(rpt[u].pred);
        u = rpt[u].pred;
      }
      for (i = backwards_path.size()-1; i>=0; i--) {
    ...
    bool dag_critical_paths(vector<vertex_label> &rpt) {
      ...
      int i, holder;    // holder for debugging purposes
      for (i=0; i<rpt.size(); i++) {
        holder = _dag_cpaths(i, rpt, vertices);
      }
  ...
  private:
    static int _dag_cpaths (int u, vector<vertex_label> &rpt, vector<vertex> &vertices) {
        if (rpt[u].state == FINISHED) {
          return rpt[u].dist;
        }
        ...
        int crit = 0, temp, pred_id;
        for (edge &e : vertices[u].incoming) {
          temp = _dag_cpaths(e.vertex_id, rpt, vertices) + e.weight;
          if (temp > crit) { crit=temp; pred_id=e.vertex_id; }
        }
        rpt[u].dist  = crit;
        rpt[u].pred  = pred_id;
        rpt[u].state = FINISHED;
  ...
  public:
    ...
    bool dag_num_paths (vector<vertex_label> &rpt) {
      ...
      rpt.clear();
      for(u=0; u<vertices.size(); u++) {
        rpt.push_back(vertex_label(0, 0, UNDISCOVERED));
      }

      for (int i=0; i<rpt.size(); i++) {
        if (rpt[i].state == UNDISCOVERED) {
          rpt[i].npaths = _in_paths(i,rpt,vertices)*_out_paths(i,rpt,vertices);
          rpt[i].state = FINISHED;
        }
        else if (rpt[i].state == DISCOVERED) {
          rpt[i].npaths = rpt[i].pred*_out_paths(i,rpt,vertices);
          rpt[i].state = FINISHED;
        }
        else if (rpt[i].state == ACTIVE) {
          rpt[i].npaths = _in_paths(i,rpt,vertices)*rpt[i].dist;
          rpt[i].state = FINISHED;
        }
        else if (rpt[i].npaths == 0) {
  ...
  private:
    static int _in_paths (int u, vector<vertex_label> &rpt, vector<vertex> &vertices) {
      if (rpt[u].state == DISCOVERED || rpt[u].state == FINISHED) 
        return rpt[u].pred;
      else if (rpt[u].state == ACTIVE)
        rpt[u].state = FINISHED;
      else      
      ...
      for (edge &e : vertices[u].incoming) {
        rpt[u].pred += _in_paths(e.vertex_id, rpt, vertices);
      }
      
      return rpt[u].pred;
    }

    static int _out_paths (int u, vector<vertex_label> &rpt, vector<vertex> &vertices) {
      if (rpt[u].state == ACTIVE || rpt[u].state == FINISHED) 
        return rpt[u].dist;
      else if (rpt[u].state == DISCOVERED)
        rpt[u].state = FINISHED;
      else
        rpt[u].state = ACTIVE;
      ...
      for (edge &e : vertices[u].outgoing) {
        rpt[u].dist += _out_paths(e.vertex_id, rpt, vertices);
      }
  ...
  public:
    ...
    bool valid_topo_order(const vector<int> &order) {
      ...
      int i, u, v, n = order.size();
      vector<int> node2pos, pos2node = order; 
      node2pos.resize(n);
      for (i=0; i<n; i++) {
        node2pos[order[i]] = i;
      }
     ...
     *
     * function:  enum_paths
     * desc:  enumerates all input-paths ending at target vertex in
     *        a DAG.
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      // your code here!
      string init_s = id2name(target);
      _enum_paths(target, paths, vertices, init_s);
      _qsort(paths);
      return true;
    }

  private: 
    static void _enum_paths (int u, vector<string> &paths, vector<vertex> &vertices, string temp) {
      if (vertices[u].incoming.size() == 0) {
      ...
      for (edge &e : vertices[u].incoming) {
        v = e.vertex_id;
        curr = vertices[v].name + " " + temp;
        _enum_paths(v, paths, vertices, curr);
      }
    }

    static void _qsort(vector<string> &paths) {
      vector<string> right;
      string pivot;
      int n = paths.size();
    ...
    static void _filter_geq(vector<string> &paths, const string &cutoff, vector<string> &right) {
			vector<string> temp = paths;
      paths.clear();
      paths.push_back(cutoff);
  ...
  public:
    ...
    void enum_spaths (std::vector<vertex_label> &rpt, int u, std::vector<string> &spaths) {
      spaths.clear();
      if(has_cycle() || u < 0 || u >= num_nodes())
        return;
      if (rpt[u].dist == -1) return;

      string init_s = id2name(u);
      _enum_spaths(u, 0, spaths, rpt[u].dist, vertices, init_s);
    }
  
  private:
    void _enum_spaths (int u, double c, vector<string> &spaths, double dist,
                       vector<vertex> &vertices, string temp) {
      if (vertices[u].incoming.size() == 0 && c == dist) {
        spaths.push_back(temp);
        return;
      }
      if (c >= dist) return; 
      ...
      for (edge &e : vertices[u].incoming) {
        ...
        _enum_spaths(v, c+1, spaths, dist, vertices, curr);
      }
    }
